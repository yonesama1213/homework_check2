...
if user_input_email:
    ...
    # --- 📖 授業管理 ---
        ...
        if st.session_state.selected_course:
            ...
            if crs_res.data:
                crs = crs_res.data[0]
                st.subheader(f"📖 {crs['grade']}年 【{crs['subject_area']}】 {crs['name']} (担当: {crs['teacher_name']})")
                b_cols = st.columns(4)
                ...
                if st.session_state.course_sub_page == "bulk":
                    h_list = supabase.table("class_master").select("grade, class_name").execute()
                    sh = st.selectbox("クラス選択", [f"{r['grade']}年{r['class_name']}" for r in h_list.data])
                    if st.button("登録実行"):
                        gv, cv = sh.split('年'); stus = supabase.table("students").select("email").eq("grade", gv).eq("class", cv).execute()
                        [supabase.table("user_courses").upsert({"user_id": s['email'], "course_id": cid}).execute() for s in stus.data]; st.rerun()
                ...
                for _, r in df_t.iterrows():
                    ...
                    if st.session_state.edit_email == m:
                        with st.form(f"et_{m}"):
                            c1, c2, c3 = st.columns(3); ln = c1.text_input("姓", value=r['last_name']); fn = c1.text_input("名", value=r['first_name']); lnf = c1.text_input("姓フリ", value=r['last_name_furi']); fnf = c2.text_input("名フリ", value=r['first_name_furi']); sub = c3.text_input("教科", value=r['subject'])
                            if st.form_submit_button("更新"): supabase.table("admins").update({"last_name":ln, "first_name":fn, "last_name_furi":lnf, "first_name_furi":fnf, "subject":sub}).eq("email", m).execute(); st.session_state.edit_email = None; st.rerun()
                    else:
